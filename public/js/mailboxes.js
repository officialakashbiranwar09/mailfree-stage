    ...
    btn.onclick = async (e) => {
      ...
      switch (action) {
        ...
        case 'login':
          ...
          if (mailbox) {
            try {
              ...
              showToast(mailbox.can_login ? '已禁止Access' : '已允许Access', 'success');
              load();
            } catch(e) { showToast('Operation failed', 'error'); }
          }
          break;
        case 'password':
          ...
          break;
        case 'delete':
          if (!confirm(`OKDeleteMailbox ${address}？`)) return;
          try {
...
function openPasswordModal(address, isDefault) {
  ...
  if (isDefault) {
    ...
    if (els.passwordModalTitle) els.passwordModalTitle.textContent = 'SettingsPassword';
    if (els.passwordModalMessage) els.passwordModalMessage.innerHTML = `为 <strong>${address}</strong> Settings新Password：`;
    ...
    // 重置Password
    if (els.passwordModalIcon) els.passwordModalIcon.textContent = '🔓';
    if (els.passwordModalTitle) els.passwordModalTitle.textContent = '重置Password';
    if (els.passwordModalMessage) els.passwordModalMessage.innerHTML = `OK将 <strong>${address}</strong> 的Password重置为defaultPassword（Mailbox Address）？`;
...
async function executePasswordAction() {
  ...
  try {
    ...
    if (currentPasswordIsDefault) {
      ...
      if (!newPwd) {
        showToast('Please enter新Password', 'error');
        return;
      }
    ...
    if (res.ok) {
      showToast(currentPasswordIsDefault ? 'PasswordSet' : 'Password已重置', 'success');
      closePasswordModal();
      load();
    } else {
    ...
    showToast('Operation failed: ' + (e.message || '未知错误'), 'error');
  } finally {
...
function openBatchModal(action, title, icon, message) {
  ...
  if (els.batchCountInfo) els.batchCountInfo.textContent = '输入Mailbox后将show count';
...
function updateBatchCount() {
  ...
  if (els.batchCountInfo) {
    els.batchCountInfo.textContent = emails.length > 0 ? `已识别 ${emails.length} 个Mailbox Address` : '输入Mailbox后将show count';
  }
  ...
  try {
    ...
    switch (currentBatchAction) {
      ...
      case 'forward':
        ...
        if (!forwardTo) { showToast('Please enter转发目标', 'error'); return; }
    ...
    showToast('Bulk Action完成', 'success');
    closeBatchModal();
    load();
  } catch (e) {
    showToast('Operation failed: ' + (e.message || '未知错误'), 'error');
...
els.logout?.addEventListener('click', async () => { try { await fetch('/api/logout', { method: 'POST' }); } catch(_) {} location.replace('/login'); });

// Bulk Action按钮
els.batchAllow?.addEventListener('click', () => openBatchModal('allow', 'Allow SelectedAccess', '✅', '输入要允许Access的Mailbox Address（每行一个或用逗号分隔）：'));
els.batchDeny?.addEventListener('click', () => openBatchModal('deny', 'Deny SelectedAccess', '🚫', '输入要禁止Access的Mailbox Address（每行一个或用逗号分隔）：'));
els.batchFavorite?.addEventListener('click', () => openBatchModal('favorite', 'Favorite Selected', '⭐', '输入要收藏的Mailbox Address（每行一个或用逗号分隔）：'));
els.batchUnfavorite?.addEventListener('click', () => openBatchModal('unfavorite', '批量Cancel收藏', '☆', '输入要Cancel收藏的Mailbox Address（每行一个或用逗号分隔）：'));
els.batchForward?.addEventListener('click', () => openBatchModal('forward', '批量Settings转发', '↪️', '输入要Settings转发的Mailbox Address（每行一个或用逗号分隔）：'));
els.batchClearForward?.addEventListener('click', () => openBatchModal('clear-forward', '批量Clear Forwarding', '🚫', '输入要Clear Forwarding的Mailbox Address（每行一个或用逗号分隔）：'));
