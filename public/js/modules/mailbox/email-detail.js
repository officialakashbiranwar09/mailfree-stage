...
export function renderEmailDetail(email) {
  if (!email) {
    return '<div class="empty-detail">请选择一封Email</div>';
  }
  ...
  if (to) metaHtml += `<span>Recipients：${to}</span>`;
...
export function renderEmailModal(email) {
  ...
  return `
    ...
    <div class="email-meta-inline">
      ...
      ${to ? `<span>Recipients：${to}</span>` : ''}
      <span>${receivedAt}</span>
      ${verificationCode ? `<span class="code-highlight" data-code="${escapeAttr(verificationCode)}" title="点击复制" style="cursor:pointer">Verify码：${escapeHtml(verificationCode)}</span>` : ''}
    </div>
    <div class="modal-body">
      ${content}
    </div>
    <div class="modal-footer">
      <button class="btn btn-danger" data-action="delete" data-email-id="${email.id}">DeleteEmail</button>
...
export function extractVerificationCode(text) {
  ...
  const keywords = '(?:Verify码|校验码|激活码|verification\\s+code|security\\s+code|otp|code)';
