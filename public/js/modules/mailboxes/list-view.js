...
export function renderMailboxListItem(mailbox, options = {}) {
  ...
  return `
    <div class="mailbox-list-item ${isPinned ? 'pinned' : ''}" data-address="${escapedAddress}">
      <div class="item-pin ${isPinned ? 'active' : ''}" data-action="pin" title="${isPinned ? 'Cancel置顶' : '置顶'}">
      ...
      <div class="item-content">
        ...
        <div class="item-meta">
          ...
          <span class="item-indicators">
            ${isFavorite ? '<span class="indicator favorite" title="Favorited"><svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor"><use href="/icons/sprites.svg#icon-star"/></svg></span>' : ''}
      ...
      <div class="item-actions">
        ...
        <button class="btn btn-sm ${isFavorite ? 'active' : ''}" data-action="favorite" title="${isFavorite ? 'Cancel收藏' : '收藏'}"><svg width="14" height="14" viewBox="0 0 24 24" fill="${isFavorite ? 'currentColor' : 'none'}" stroke="currentColor" stroke-width="2"><use href="/icons/sprites.svg#icon-star-${isFavorite ? '' : 'empty'}"/></svg></button>
        <button class="btn btn-sm" data-action="forward" title="Forward Settings"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><use href="/icons/sprites.svg#icon-forward"/></svg></button>
        <button class="btn btn-sm" data-action="toggle-login" title="${canLogin ? '禁止Access' : '允许Access'}"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><use href="/icons/sprites.svg#icon-key"/></svg></button>
        <button class="btn btn-sm danger" data-action="delete" title="Delete"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><use href="/icons/sprites.svg#icon-trash"/></svg></button>
      </div>
    </div>
  `;
}
...
export function renderTableHeader() {
  return `
    <div class="table-header">
      ...
      <div class="col-time">Create时间</div>
