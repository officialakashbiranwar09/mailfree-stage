...
/**
 * 渲染网格卡片（使用原始 CSS 类名）
 * 操作按钮：复制、置顶、Settings转发、收藏（2x2 布局）
 * 点击卡片跳转Mailbox
 * @param {object} m - Mailbox数据
 * @returns {string}
 */
export function renderCard(m) {
  ...
  return `
    <div class="mailbox-card" data-address="${addr}" data-id="${m.id}" data-action="jump">
      ...
      <div class="line pwd">${m.password_is_default ? '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><use href="/icons/sprites.svg#icon-unlock"/></svg> defaultPassword' : '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><use href="/icons/sprites.svg#icon-lock"/></svg> 已设Password'}</div>
      <div class="line login">${m.can_login ? '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><use href="/icons/sprites.svg#icon-check-circle"/></svg> 可Access' : '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><use href="/icons/sprites.svg#icon-x-circle"/></svg> 禁止Access'}</div>
      ...
      <div class="actions">
        ...
        <button class="btn-icon" data-action="password" title="${m.password_is_default ? 'SettingsPassword' : '重置Password'}"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><use href="/icons/sprites.svg#icon-key"/></svg></button>
        <button class="btn-icon" data-action="forward" title="Settings转发"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><use href="/icons/sprites.svg#icon-forward"/></svg></button>
        <button class="btn-icon ${m.is_favorite ? 'active' : ''}" data-action="favorite" title="${m.is_favorite ? 'Cancel收藏' : '收藏'}"><svg width="14" height="14" viewBox="0 0 24 24" fill="${m.is_favorite ? 'currentColor' : 'none'}" stroke="currentColor" stroke-width="2"><use href="/icons/sprites.svg#icon-star-${m.is_favorite ? '' : 'empty'}"/></svg></button>
      </div>
    </div>`;
}
  ...
  return `
    <div class="mailbox-list-item" data-address="${addr}" data-id="${m.id}">
      ...
      <div class="mailbox-info">
        ...
        <div class="meta">
          ...
          ${forward
            ...
            : '<span class="meta-status meta-forward-empty" title="Not Set转发">—</span>'}
        </div>
      </div>
      <div class="list-actions">
        ...
        <button class="btn" data-action="jump" title="查看Email"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><use href="/icons/sprites.svg#icon-mail"/></svg></button>
        <button class="btn" data-action="forward" title="Forward Settings"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><use href="/icons/sprites.svg#icon-forward"/></svg></button>
        <button class="btn ${m.is_favorite ? 'active' : ''}" data-action="favorite" title="${m.is_favorite ? 'Cancel收藏' : '收藏'}">${m.is_favorite ? '<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><use href="/icons/sprites.svg#icon-star"/></svg>' : '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><use href="/icons/sprites.svg#icon-star-empty"/></svg>'}</button>
        <button class="btn" data-action="login" title="${m.can_login ? '禁止Access' : '允许Access'}">${m.can_login ? '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><use href="/icons/sprites.svg#icon-lock"/></svg>' : '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><use href="/icons/sprites.svg#icon-unlock"/></svg>'}</button>
        <button class="btn" data-action="password" title="${m.password_is_default ? 'SettingsPassword' : '重置Password'}">🔑</button>
