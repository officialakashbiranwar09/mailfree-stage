...
/**
 * 从文本中提取Verify码
 * @param {string} text - 文本Message
 * @returns {string}
 */
export function extractCode(text) {
  if (!text) return '';
  const keywords = '(?:Verify码|校验码|激活码|one[-\\s]?time\\s+code|verification\\s+code|security\\s+code|two[-\\s]?factor|2fa|otp|login\\s+code|code)';
