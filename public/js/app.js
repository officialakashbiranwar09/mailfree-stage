...
function showCountdown() { if (els.listLoading) { els.listLoading.innerHTML = `<span class="countdown-icon">⏱</span>Refresh in ${countdown}s`; els.listLoading.style.display = 'flex'; }}
...
async function loadMailboxes(opts = {}) {
  ...
  try {
    ...
    try { const q = document.getElementById('quota'); if (q) q.textContent = `${total} Mailbox`; } catch(_) {}
  } catch(_) {}
...
function updateMailboxInfoUI(info) {
  ...
  if (els.favoriteIcon && els.favoriteText) {
    ...
    els.favoriteText.textContent = info.is_favorite ? 'Favorited' : 'Favorite Mailbox';
  }
}
...
window.showSentEmail = async (id) => { try { const r = await api(`/api/sent/${id}`); showSentEmailDetail(await r.json(), els); } catch(e) { showToast(e.message || 'Loading failed', 'error'); }};
...
if (els.tabSent) els.tabSent.onclick = () => { setView(true); els.tabSent.classList.add('active'); els.tabInbox?.classList.remove('active'); if (els.boxTitle) els.boxTitle.textContent = 'Sent Mail'; if (els.boxIcon) els.boxIcon.textContent = '📤'; resetPager(els); refresh(); };
...
(async () => {
  ...
  if (!s) { clearCurrentMailbox(); stopAutoRefresh(); location.replace('/login'); return; }
  ...
  try { const qr = await api('/api/user/quota'); const q = await qr.json(); const el = document.getElementById('quota'); if (el && q) { el.textContent = isAdmin() ? `${q.total || 0} Mailbox` : `${q.used || 0} / ${q.limit || 0}`; }} catch(_) {}
  await loadMailboxes();
  
  // 优先使用 URL 参数中的Mailbox，其次使用本地存储的上次Mailbox
  const urlParams = new URLSearchParams(window.location.search);
  const urlMailbox = urlParams.get('mailbox');
  if (urlMailbox) {
    await window.selectMailbox(urlMailbox);
    // 清除 URL 参数，避免Refresh时重复选择
