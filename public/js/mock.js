(function(global){
  ...
  function buildMockEmails(count){
    ...
    return Array.from({length: count||6}).map((_, i) => {
      ...
      return {
        ...
        is_read: i>1,
        content: `您好，您正在体验Demo Mode。Verify码: ${code} ，请在 5 分钟内完成Verify。`,
        html_content: `<p>您好，您正在体验 <strong>Demo Mode</strong>。</p><p><strong>Verify码: ${code}</strong></p>`
      };
    });
  }
  ...
  function buildMockEmailDetail(id){
    ...
    return {
      ...
      subject: `演示Email Content（Verify码 ${code}）`,
      received_at: formatTs(Date.now()),
      content: `这是Demo Mode下的Email Content，仅用于展示界面效果。Verify码：${code}`,
      html_content: `<p><strong>Demo Mode</strong>：该Message为模拟数据。</p><p>Verify码：<strong>${code}</strong></p>`
    };
  }
