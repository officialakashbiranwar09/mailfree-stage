      ...
      if (list.length > 0) {
        container.innerHTML = list.map(m => `
          <div class="mailbox-item" data-address="${m.address}" data-href="/?mailbox=${encodeURIComponent(m.address)}">
            ...
            <button class="btn danger" data-action="unassign">CancelAssign</button>
          </div>
        `).join('');
        ...
        container.querySelectorAll('[data-action="unassign"]').forEach(btn => {
          btn.onclick = async (e) => {
            ...
            const confirmed = await showConfirm(`OKCancelAssign Mailbox ${address}？`);
            if (!confirmed) return;

            try {
              await unassignMailbox(currentViewingUser.username, address);
              showToast('已CancelAssign', 'success');
              loadUserMailboxes();
            } catch(e) { showToast('CancelAssign失败', 'error'); }
          };
        });
      } else {
        container.innerHTML = '';
...
async function handleCreateUser() {
  ...
  if (!username || !password) {
    showToast('Username和Password不能为空', 'error');
    return;
  }
  
  try {
    await createUser({ username, password, role });
    showToast('UserCreated successfully', 'success');
...
async function handleAssignMailbox() {
  ...
  if (!username) {
    showToast('Please enterUsername', 'error');
    return;
  }
  
  if (!addressText) {
    showToast('Please enterMailbox Address', 'error');
    return;
  }
  ...
  if (addresses.length === 0) {
    showToast('Please enter有效的Mailbox Address', 'error');
    return;
  }
  ...
  try {
    ...
    if (successCount > 0 && failCount === 0) {
      showToast(`成功Assign ${successCount} 个Mailbox`, 'success');
    } else if (successCount > 0 && failCount > 0) {
      showToast(`成功 ${successCount} 个，失败 ${failCount} 个`, 'warning');
    } else {
      showToast('Assign失败', 'error');
    }
    ...
    // 如果当前有查看的User且Username匹配，RefreshMailbox列表
...
async function handleUnassignMailbox() {
  ...
  try {
    ...
    if (successCount > 0 && failCount === 0) {
      showToast(`成功CancelAssign ${successCount} 个Mailbox`, 'success');
    } else if (successCount > 0 && failCount > 0) {
      showToast(`成功 ${successCount} 个，失败 ${failCount} 个`, 'warning');
    } else {
      showToast('CancelAssign失败', 'error');
    }
...
els.logout?.addEventListener('click', async () => { try { await api('/api/logout', { method: 'POST' }); } catch(_) {} location.replace('/login'); });
...
els.editDelete?.addEventListener('click', async () => {
  ...
  const confirmed = await showConfirm(`OKDeleteUser "${currentViewingUser.username}" 吗？此操作不可恢复。`);
  if (!confirmed) return;
  
  try {
    await deleteUser(currentViewingUser.id);
    showToast('User已Delete', 'success');
    els.editModal?.classList.remove('show');
    loadUsers();
